...
class Lomb_Scargle(object):
    def __init__(self, x, y, Tmax=500):
        ...
        frequency, self.power = ls.autopower(
            minimum_frequency=1 / Tmax, maximum_frequency=1 / 2, samples_per_peak=20
        )

        self.periods = 1 / frequency

        self.best_T = self.periods[np.argmax(self.power)]
    ...
    def dtrend(self):
        ...
        fit = b.CurveFit(self.x, self.y, period=self.best_T)
...
def plot_(ax, ls, x, y, best_period):
    ...
    phase_sorted, amplitude, y_sorted, yerr = phase_folded(ls, x, y, best_period)
    inset_ax.errorbar(phase_sorted, y_sorted, yerr=yerr, fmt="ko", markersize=3)

    inset_ax.plot(phase_sorted, amplitude, "b-")

    inset_ax.set(xlabel="phase", ylabel="mag")
