...
def heights_frequency(dn, days, col, site="saa", reindex=True):
...
def fix_drift(df):
    ...
    df.rename(columns={"vzp": "vp"}, inplace=True)
...
def vertical_drift(dn, days, reindex=True, site="saa"):
...
def plot_long_term_series(dn, days, site="saa"):
    ...
    fig, ax = plt.subplots(nrows=2, ncols=2, dpi=300, sharex=True, figsize=(16, 8))

    titles = {"fza": "Fortaleza", "saa": "São Luís"}
    plt.subplots_adjust(hspace=0.02)
    df = heights_frequency(dn, days, site=site, reindex=False)
    ...
    for i, col in enumerate(["hF", "foF2"]):
        ...
        ax[i, 0].errorbar(
            mean.index, mean, yerr=std, marker="o", markersize=10, color="k"
        )

        ax[i, 0].set(ylabel=labels[i], ylim=limits[i], xlim=[df.index[0], df.index[-1]])

    b.format_month_axes(ax[1, 1], month_locator=3, pad=60)
    ...
    for j, col in enumerate(["vp", "time"]):
        ...
        ax[j, 1].errorbar(
            mean.index, mean, yerr=std, color="k", marker="o", markersize=10
        )
        ax[j, 1].set(ylabel=labels[j], ylim=limits[j], xlim=[df.index[0], df.index[-1]])
